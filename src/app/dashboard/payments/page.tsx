...
export default async function PaymentsPage({
  ...
  const [{ data: suppliers }, { data: milestones }, { data: variationOrders }] = await Promise.all([
    companyId
      ? supabase
    ...
    companyId
      ? supabase
          ...
          .select('id, supplier_id, title, description, milestone_amount, approved_invoice_total, payment_date, payment_reference')
          .eq('company_id', companyId)
        ...
        <>
          <div style={{ ...panelStyle, marginBottom: '1.5rem', maxWidth: '960px' }}>
            <div style={fieldWrapStyle}>
              ...
              <PaymentSupplierFilter suppliers={supplierRows.map(({ id, name }) => ({ id, name }))} selectedSupplierId={selectedSupplierId} />
            </div>
          </div>
            ...
            <div style={{ ...panelStyle, marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: '0.8rem' }}>
                <div style={{ minWidth: 0 }}>
                  <h2 style={{ margin: 0, fontSize: '1.08rem', color: colors.ink, fontWeight: 800 }}>{selectedSupplier.name}</h2>
              ...
              <div style={{ marginBottom: '0.75rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', marginBottom: '0.45rem', fontSize: '0.88rem', color: colors.muted }}>
                  ...
                  <strong style={{ color: colors.ink }}>{approvedPercent.toFixed(0)}%</strong>
                </div>
          ...
          <div style={{ ...summaryGridStyle, marginBottom: '1.5rem' }}>
            ...
            <SummaryCard
              ...
              value={String(
                selectedMilestones.filter((item) => item.payment_date || item.payment_reference).length +
                  selectedVos.filter((item) => item.payment_date || item.payment_reference).length
              )}
            />
          </div>
                  ...
                  {selectedMilestones.length === 0 ? (
                    ...
                    selectedMilestones.map((item) => (
                      <tr key={item.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                        ...
                        <BodyCell>{formatCurrency(Number(item.milestone_amount || 0) - Number(item.approved_invoice_total || 0))}</BodyCell>
